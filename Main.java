...
//Item class with properties name & price of item
class Item {
  String name;
  int price;
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //Input stream to read input.txt  
    FileInputStream fis=new FileInputStream("input.txt");      
    Scanner sc=new Scanner(fis);

    //Reading no of employees
    int number_of_employees = Integer.parseInt(sc.nextLine().split(":")[1].trim());
    sc.nextLine(); sc.nextLine(); sc.nextLine();

    ArrayList<Item> goodies_items = new ArrayList<Item>();

    //Reading Item name & price
    ...
    //Traversing Item list to find least difference between two items
